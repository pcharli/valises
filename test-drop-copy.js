...
boites.forEach(col => {
    ...
    let objets = produits.filter(el => el.boite[0] == boiteName)
    objets.forEach(el => {
        col.innerHTML += `
...
boites.forEach(col => {
    ...
    col.addEventListener('drop', function (e) {
        ...
        this.appendChild(clonedElement, this)

        dragSrcEl.classList.remove('dragged')
        clonedElement.classList.remove('dragged')
        produits[produits.findIndex(el => el.key == clonedElement.id)].boite.push(colName)
